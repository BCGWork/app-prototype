...
source("d3Templates.R")

d3ForceNetworkAdapted <- function(Links, Nodes, Source, Target, Value = NULL, NodeID,
                           Group, Count, height = 600, width = 900, fontsize = 7, linkDistance = 50,
                           ...
                           d3Script = "http://d3js.org/d3.v3.min.js")
{
  ...
  NodesDF <- data.frame(Nodes[, NodeID], Nodes[, Group], Nodes[, Count])
  names(NodesDF) <- c("name", "group", "count")
